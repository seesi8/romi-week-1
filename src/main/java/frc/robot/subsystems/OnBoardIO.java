...
public class OnBoardIO extends SubsystemBase {
  ...
  //DIO ?
  private final DigitalOutput m_newLed;
  ...
  public OnBoardIO(ChannelMode dio1, ChannelMode dio2) {
    if (dio1 == ChannelMode.INPUT) {
      System.out.println("dio is input");
      m_buttonB = new DigitalInput(1);
      m_greenLed = null;
    } else {
      System.out.println("DIO is output");
      m_buttonB = null;
  ...
  public void setNewLed(boolean value) {
    if (m_greenLed != null) {
      m_newLed.set(value);
    } else {
      double currentTime = Timer.getFPGATimestamp();
